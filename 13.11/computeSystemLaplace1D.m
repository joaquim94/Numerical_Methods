function [K,f] = computeSystemLaplace1D(X,T,referenceElement,source)
...
%Bucle en elements
    for e=1:nOfElements
        ...
        [Ke,fe] = elementalComputations(Xe,referenceElement,source);
        K(Te,Te) = K(Te,Te) + Ke;
        f(Te) = f(Te) + fe;
    ...
    function [Ke,fe] = elementalComputations(Xe,referenceElement,source)
    %h = Xe(2)-Xe(1);
    %Ke = (1/h)*[1 -1; -1 1];
    %fe = h*[-1;-1];
