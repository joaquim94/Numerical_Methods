...
MY_FLOAT **taylor_coefficients_eq_os(MY_FLOAT t, MY_FLOAT *x, int order);

MY_FLOAT **taylor_coefficients_eq_osA(MY_FLOAT t, MY_FLOAT *x, int order, int reuse_last_computation);
int       taylor_step_eq_os(MY_FLOAT *ti, MY_FLOAT *x, int dir, int step_ctl,
...
int taylor_step_eq_os(MY_FLOAT *ti,
                 MY_FLOAT *x,
                 int      dir,
                 int      step_ctl,
                 double   log10abserr,
 ...
 * 1: ok, and ti=endtime.  */
{
  MY_FLOAT **taylor_coefficients_eq_os(MY_FLOAT, MY_FLOAT*, int);
  MY_FLOAT **taylor_coefficients_eq_osA(MY_FLOAT, MY_FLOAT*, int, int);
  int compute_order_1_eq_os(double, double, double, int*);
  int comp_order_other_eq_os(double, double, double);
  double compute_stepsize_1_eq_os(MY_FLOAT**, int, double, int);
  double compute_stepsize_2_eq_os(MY_FLOAT**, int, double, int);
  double comp_stepsize_other_eq_os(MY_FLOAT**, int, int, double, double, double);
...
*/
  switch(step_ctl)
    ...
    case 0: /* no step size control, fixed degree; both given by the user */
      ...
      break;
    case 1:
      nt=compute_order_1_eq_os(xnorm,log10abserr,log10relerr,&flag_err);
      break;
    case 2:
      nt=compute_order_1_eq_os(xnorm,log10abserr,log10relerr,&flag_err);
      break;
    case 3:
      nt=comp_order_other_eq_os(xnorm,log10abserr,log10relerr);
      break;
    default:
...
*/
  if(step_ctl != 0) {
    s=taylor_coefficients_eq_osA(*ti,x,nt,1);
  } else {
    s=taylor_coefficients_eq_os(*ti,x,nt); 
 }
...
*/
  switch(step_ctl)
    ...
    case 0: /* no step size control (fixed step size, given by the user) */
      ...
      break;
    case 1:
      dh=compute_stepsize_1_eq_os(s,nt,xnorm,flag_err);
      MakeMyFloatA(h, dh);
      break;
    case 2:
      dh=compute_stepsize_2_eq_os(s,nt,xnorm,flag_err);
      MakeMyFloatA(h, dh);
      break;
    case 3:
      dh=comp_stepsize_other_eq_os(s,_N_DIM_,nt,xnorm,log10abserr,log10relerr);
      MakeMyFloatA(h, dh);
      break;
    default:
      fprintf(stderr, "taylor_step: undefined step size control.\n");
...
int compute_order_1_eq_os(double xnorm, double log10abserr, double log10relerr, int* flag_err)
/*
...
double compute_stepsize_1_eq_os(MY_FLOAT **s, int nt, double xnorm, int flag_err)
/*
 ...
 */
{
  double double_log_MyFloat_eq_os(MY_FLOAT x);
...
*/
  if (MyFloatA_LE_B(v1,of) && MyFloatA_GE_B(v1,uf))
    ...
    {
      lnv1=double_log_MyFloat_eq_os(v1);
    }
  if (MyFloatA_LE_B(v2,of) && MyFloatA_GE_B(v2,uf))
    ...
    {
      lnv2=double_log_MyFloat_eq_os(v2);
    }
/*
...
double compute_stepsize_2_eq_os(MY_FLOAT **s, int nt, double xnorm, int flag_err)
/*
 * it looks for a step size for an expansion up to order nt. this
 * function requires that nt is the value computed by
...
int comp_order_other_eq_os(double lnxnorm, double log10abserr, double log10relerr){
  puts("---");
...
double comp_stepsize_other_eq_os(MY_FLOAT **s, int nd, int nt, double xnorm, double log10abserr, double log10relerr) {
...
void taylor_f77_eq_os__(MY_FLOAT *ti,
                           MY_FLOAT *x,
                           int      *dir,
                           int      *step_ctl,
                           double   *log10abserr,
...
*/
{
  *flag=taylor_step_eq_os(ti,x,*dir,*step_ctl,*log10abserr,*log10relerr,
                             endtime,ht,order);
  return;
}
...
#include <stdio.h>
#include <stdlib.h>
MY_FLOAT **taylor_coefficients_eq_osA(MY_FLOAT t, MY_FLOAT *x, int order, int rflag)
{
   /* input: 
    ...
    static MY_FLOAT     *_jz_jet[3],  *_jz_save = NULL, *_jz_oneOverN=NULL,*_jz_theNs=NULL;
    ...
    if(_jz_maxOrderUsed < order )  { 
    	 ...
    	 if(_jz_ginitialized) {
    	    for(_jz_i=0; _jz_i<(_jz_oorder+1)*(3); _jz_i++) { ClearMyFloat(_jz_save[_jz_i]);} free(_jz_save);
    	 }
    	 _jz_save = (MY_FLOAT *)malloc((order+1)* 3 *sizeof(MY_FLOAT));
    	 for(_jz_i=0; _jz_i<(order+1)*(3); _jz_i++) { InitMyFloat(_jz_save[_jz_i]);}
    	 for(_jz_j = 0, _jz_k = 0; _jz_j < 3 ;  _jz_j++, _jz_k += order+1) { _jz_jet[_jz_j] =& (_jz_save[_jz_k]); }
    ...
    if(rflag == 0) {
	 ...
	 /* negate: v_006=(-v_002) */
	 NegateMyFloatA(_jz_jet[2][0],_jz_jet[0][0]);

	 /* the first derivative of state variables */
	 /* state variable 0: */
	 AssignMyFloat(_jz_jet[0][1], _jz_jet[1][0]);
	 /* state variable 1: */
	 AssignMyFloat(_jz_jet[1][1], _jz_jet[2][0]);
	}

	 /* compute the kth order derivatives of all vars */
	 for(_jz_k = _jz_lastOrder; _jz_k < order; _jz_k++) {
		 /* derivative for tmp variables */
		 /* negation: v_006=(-v_002) */
		 NegateMyFloatA(_jz_jet[2][_jz_k], _jz_jet[0][_jz_k]);
		 /* derivative of state variables */
		 _jz_m = _jz_k+1;
		 /* state variable 0: */
		 DivideMyFloatByInt(_jz_jet[0][_jz_m], _jz_jet[1][_jz_k], _jz_m);
		 /* state variable 1: */
		 DivideMyFloatByInt(_jz_jet[1][_jz_m], _jz_jet[2][_jz_k], _jz_m);
		 _jz_initialized=1;
	 }
    _jz_lastOrder = order;
    _jz_ginitialized=1;
...
MY_FLOAT **taylor_coefficients_eq_os(MY_FLOAT t, MY_FLOAT *x, int order)
{
    return(taylor_coefficients_eq_osA(t,x,order,0));
}
...
=======                                                                      ======
=======                         Final Variable List                          ======
	    (3 + 0) vars, (0 + 0) cvars and (0 + 0) ivars 
=======                                                                      ======
===================================================================================
	v_002 (state variable)
	v_003 (state variable)
	v_006 = (-v_002)                                 (2 0)
===================================================================
=========                                                  ========
